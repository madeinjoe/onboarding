...
class CustomInput
{
    public static function renderInput(array $attrib = [])
    {
        if (!$attrib or count($attrib) <= 0) {
            ...
            switch ($attrib['type']) {
                case 'number':
                    echo '<input style="width: 100%; border: 1px solid rgba(209, 213, 219, 1); padding: 0.375rem 0.5rem; font-size: 1rem; line-height: 1.5rem; font-weight: 400;" type="number" id="ci_' . $attrib['meta-key'] . '" name="' . $attrib['meta-key'] . '" min="' . ($attrib['min'] ?? 0) . '" max="' . ($attrib['max'] ?? 100) . '" step="' . ($attrib['step'] ?? '0.01') . '" value="' . ($attrib['meta-value'] ?? '') . '" />';
                    break;
                case 'text-area':
                    echo '<textarea style="width: 100%; border: 1px solid rgba(209, 213, 219, 1); padding: 0.375rem 0.5rem; font-size: 1rem; line-height: 1.5rem; font-weight: 400;" id="ci_' . $attrib['meta-key'] . '" name="' . $attrib['meta-key'] . '" rows="4">' . $attrib['meta-value'] . '</textarea>';
                    break;
                default:
                    echo '<input style="width: 100%; border: 1px solid rgba(209, 213, 219, 1); padding: 0.375rem 0.5rem; font-size: 1rem; line-height: 1.5rem; font-weight: 400;" type="text" id="ci_' . $attrib['meta-key'] . '" name="' . $attrib['meta-key'] . '" placeholder="' . ($attrib['placeholder'] ?? 'Please insert your input...') . '" value="' . $attrib['meta-value'] . '"></input>';
            }
            echo '</div>';
        }
    }

    public static function renderAllInput(array $metas = [], Bool $with_nonce = false, $nonce_action = -1, $nonce_name = "_custom_mb")
    {
